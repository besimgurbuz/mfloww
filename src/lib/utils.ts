...
export function formatTimePassedSince(date: Date | string | number): string {
...
export function formatTimeRemainingUntil(date: Date | string | number): string {
  const targetDate = new Date(date)
  const now = new Date()
  const diffInMinutes = Math.floor(
    (targetDate.getTime() - now.getTime()) / 60000
  )

  if (diffInMinutes < 0) return "already passed"
